...
namespace griffon
{
  ...
  void
  CombustionKernels::production_rates (const double &temperature, const double &density, const double *y,
  ...
  void
  CombustionKernels::production_rates (const double &temperature, const double &density, const double &mmw,
        ...
        case RateType::TROE:
          ...
          switch (rxnData.troeForm)
          {
            case TroeTermsPresent::T123:
              logFCent = std::log10 (
                  (1 - troe[0]) * std::exp (-T / troe[1]) + troe[0] * std::exp (-T / troe[2])
                      + std::exp (-invT * troe[3]));
              break;
