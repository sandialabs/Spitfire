...
namespace griffon {
void CombustionKernels::prod_rates_sens_dense(const double &temperature, const double &density, const double &mmw, const double *y,
          ...
          * (gTroe / fCent * dfCentdT
              + logfcent
                  * (-2.0 * gTroe * gTroe / ln10 * aTroe / (bTroe * bTroe * bTroe)
                      * ((bTroe + 0.14 * aTroe) * (ARRHENIUS_SENS_OVER_K( kPCoefs ) - ARRHENIUS_SENS_OVER_K(kCoefs))
...
//          nsTmp = (-2.0 / (1. + 1. / pr) * fTroe * logfcent * gTroe * gTroe / ln10 * aTroe / (bTroe * bTroe * bTroe) * (bTroe + 0.14 * aTroe) + pr * fTroe / ((1. + pr) * (1 + pr)));
      nsTmp = kp_over_kf
          * (-2.0 / (1. + pr) * fTroe * logfcent * gTroe * gTroe / ln10 * aTroe / (bTroe * bTroe * bTroe) * (bTroe + 0.14 * aTroe)
              + fTroe / ((1. + pr) * (1 + pr)));
      dCtbafdrho = nsTmp * baseEff * invM;
      for (int i = 0; i < n_tb; ++i)
        dCtbafdrho += nsTmp * THD_BDY(i);
