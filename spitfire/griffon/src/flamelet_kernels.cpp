...
namespace griffon
{
  ...
  void
  CombustionKernels::flamelet_stencils (const double *dz, const int &nzi, const double *dissipationRate,
  ...
  void
  CombustionKernels::flamelet_jac_indices (const int &nzi, int *out_row_indices, int *out_col_indices) const
  {
    const int nSpec = mechanismData.phaseData.nSpecies;
    int idx = 0;
    // diagonal blocks
  ...
  void
  CombustionKernels::flamelet_process_density (const double *state, const double pressure, const int nzi,
                                               double *out_density) const
  {
    const int nSpec = mechanismData.phaseData.nSpecies;
    for (int i = 0; i < nzi; ++i)
    {
  ...
  void
  CombustionKernels::flamelet_process_enthalpy (const double *state, const int nzi, double *out_enthalpy) const
  {
    const int nSpec = mechanismData.phaseData.nSpecies;
    for (int i = 0; i < nzi; ++i)
    {
  ...
  void
  CombustionKernels::flamelet_process_energy (const double *state, const int nzi, double *out_energy) const
  {
    const int nSpec = mechanismData.phaseData.nSpecies;
    for (int i = 0; i < nzi; ++i)
    {
  ...
  void
  CombustionKernels::flamelet_process_cp (const double *state, const int nzi, double *out_cp) const
  {
    const int nSpec = mechanismData.phaseData.nSpecies;
    for (int i = 0; i < nzi; ++i)
    {
  ...
  void
  CombustionKernels::flamelet_process_cv (const double *state, const int nzi, double *out_cv) const
  {
    const int nSpec = mechanismData.phaseData.nSpecies;
    for (int i = 0; i < nzi; ++i)
    {
  ...
  void
  CombustionKernels::flamelet_process_mole_fractions (const double *state, const int nzi, double *out_molefracs) const
  {
    const int nSpec = mechanismData.phaseData.nSpecies;
    for (int i = 0; i < nzi; ++i)
    {
  ...
  void
  CombustionKernels::flamelet_process_isobaric_reactor_rhs (const double *state, const double pressure, const int nzi,
...
// ------------------------------------------------------------------------------------------------
  ...
  void
  CombustionKernels::flamelet_rhs (const double *state, const double &pressure, const double *oxyState,
  ...
  void
  CombustionKernels::flamelet_jacobian (const double *state, const double &pressure, const double *oxyState,
                                        const double *fuelState, const bool adiabatic, const double *T_convection,
                                        const double *h_convection, const double *T_radiation,
                                        const double *h_radiation, const int &nzi, const double *cmajor,
                                        const double *csub, const double *csup, const double *mcoeff,
                                        const double *ncoeff, const double *chi, const bool compute_eigenvalues,
                                        const double diffterm, const bool scale_and_offset, const double prefactor,
    ...
    for (int iz = 0; iz < nzi; ++iz)
    {
      ...
      if (!adiabatic)
      {
        ...
        if (use_scaled_heat_loss)
        {
          ...
          primJac[nSpec] -= invCp * cpsensT[iz] * q
              + invRhoCp * (hc / (maxT - Tc) + 4. * hr / (maxT4 - Tr4) * 5.67e-8 * T * T * T);
        }
        else
        {
