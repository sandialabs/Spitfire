...
namespace griffon
{
  ...
  void
  CombustionKernels::chem_rhs_isobaric (const double &rho, const double &cp, const double *h, const double *w,
  ...
  void
  CombustionKernels::heat_rhs_isobaric (const double &temperature, const double &rho, const double &cp,
  ...
  void
  CombustionKernels::mass_rhs_isobaric (const double *y, const double *enthalpies, const double *inflowEnthalpies,
                                        ...
                                        double *out_rhs, double *out_primJac) const
  {
    ...
    // T column
    out_primJac[nSpec] = -invRhoCp
        * (griffon::blas::inner_product (nSpec, &wsens[nSpec + 1], h) + griffon::blas::inner_product (nSpec, w, cpi))
  ...
  void
  CombustionKernels::reactor_rhs_isobaric (const double *state, const double &pressure, const double &inflowTemperature,
                                           ...
                                           const bool open, double *out_rhs) const
  {
    ...
    switch (heatTransferOption)
    {
      ...
      case 1:  // isothermal
        ...
        break;
      case 2:  // diathermal
        heat_rhs_isobaric (temperature, rho, cp, fluidTemperature, surfTemperature, hConv, epsRad,
                           surfaceAreaOverVolume, &heatTransferRate);
  ...
  void
  CombustionKernels::reactor_jac_isobaric (const double *state, const double &pressure, const double &inflowTemperature,
                                           const double *inflowY, const double &tau, const double &fluidTemperature,
                                           const double &surfTemperature, const double &hConv, const double &epsRad,
                                           const double &surfaceAreaOverVolume, const int heatTransferOption,
  ...
  void
  CombustionKernels::transform_isobaric_primitive_jacobian (const double &rho, const double &pressure,
                                                            ...
                                                            const double *primJac, double *out_jac) const
  {
    ...
    for (int k = 0; k < nSpec - 1; ++k)
    {
      for (int i = 0; i < nSpec; ++i)
      {
        out_jac[(1 + k) * nSpec + i] = primJac[(2 + k) * nSpec + i]
            + negRhoMmw * (invMolecularWeights[k] - invMolecularWeights[nSpec - 1]) * primJac[i];
      }
    }
  }
}
