...
namespace griffon
{
  ...
  void
  CombustionKernels::flamelet2d_rhs (const double *state, const double &pressure, const int &nx, const int &ny,
                                     ...
                                     const double *ycb, const double *yct, double *out_rhs) const
  {
    ...
    for (int ix = 1; ix < nx - 1; ++ix)
    {
      ...
      for (int iy = 1; iy < ny - 1; ++iy)
      {
        ...
        {
          out_rhs[ij + iq] += xcr[isx + iq] * state[ip1j + iq] + xcl[isx + iq] * state[im1j + iq]
              + xcp[isx + iq] * state[ij + iq] + yct[isy + iq] * state[ijp1 + iq] + ycb[isy + iq] * state[ijm1 + iq]
              + ycp[isy + iq] * state[ij + iq];
        }
      }
      ...
      for (int iq = 0; iq < nq; ++iq)
      {
        out_rhs[ij_1 + iq] += xcr[isx + iq] * state[ip1j_1 + iq] + xcl[isx + iq] * state[im1j_1 + iq]
            + xcp[isx + iq] * state[ij_1 + iq] + rhs_1[iq];
        out_rhs[ij_2 + iq] += xcr[isx + iq] * state[ip1j_2 + iq] + xcl[isx + iq] * state[im1j_2 + iq]
            + xcp[isx + iq] * state[ij_2 + iq] + rhs_2[iq];
      }
    }
    ...
    for (int iy = 1; iy < ny - 1; ++iy)
    {
      ...
      for (int iq = 0; iq < nq; ++iq)
      {
        out_rhs[ij_1 + iq] += yct[isy + iq] * state[ijp1_1 + iq] + ycb[isy + iq] * state[ijm1_1 + iq]
            + ycp[isy + iq] * state[ij_1 + iq] + rhs_1[iq];
        out_rhs[ij_2 + iq] += yct[isy + iq] * state[ijp1_2 + iq] + ycb[isy + iq] * state[ijm1_2 + iq]
            + ycp[isy + iq] * state[ij_2 + iq] + rhs_2[iq];
      }
    }
  ...
  void
  CombustionKernels::flamelet2d_factored_block_diag_jacobian (const double *state, const double &pressure,
                                                              const int &nx, const int &ny, const double *xcp,
                                                              const double *ycp, const double &prefactor,
                                                              double *out_values, double *out_factors,
                                                              int *out_pivots) const
  {
    const int nq = mechanismData.phaseData.nSpecies;
    const int nyq = ny * nq;
    ...
    for (int ix = 1; ix < nx - 1; ++ix)
    {
      ...
      for (int iy = 1; iy < ny - 1; ++iy)
      {
        ...
        double cpi[nq], cpisensT[nq], rhsTemp[nq], enthalpies[nq], w[nq], wsens[(nq + 1) * (nq + 1)], primJac[nq
            * (nq + 1)];
        double jac[nqq];
        const double T = state[ij];
        double y[nq];
  ...
  void
  CombustionKernels::flamelet2d_offdiag_matvec (const double *vec, const int &nx, const int &ny, const double *xcp,
                                                ...
                                                double *out_vec) const
  {
    ...
    for (int ix = 1; ix < nx - 1; ++ix)
    {
      ...
      for (int iy = 1; iy < ny - 1; ++iy)
      {
        ...
        for (int iq = 0; iq < nq; ++iq)
        {
          out_vec[ij + iq] += prefactor
              * (xcr[isx + iq] * vec[ip1j + iq] + xcl[isx + iq] * vec[im1j + iq] + yct[isy + iq] * vec[ijp1 + iq]
                  + ycb[isy + iq] * vec[ijm1 + iq]);
        }
      }
      const int ij_1 = ix * nyq;
      const int im1j_1 = ij_1 - nyq;
  ...
  void
  CombustionKernels::flamelet2d_matvec (const double *vec, const int &nx, const int &ny, const double *xcp,
  ...
  void
  CombustionKernels::flamelet2d_block_diag_solve (const int &nx, const int &ny, const double *factors,
                                                  const int *pivots, const double *b, double* out_x) const
  {
    const int nq = mechanismData.phaseData.nSpecies;
    const int nxy = nx * ny;
    const int nqq = nq * nq;
