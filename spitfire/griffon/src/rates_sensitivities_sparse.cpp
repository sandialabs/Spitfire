...
{
  void
  CombustionKernels::prod_rates_sens_sparse (const double &temperature, const double &density, const double &mmw,
      ...
      switch (polyType)
      {
        case CpType::NASA7:
          if (T <= c[0])
          {
            ...
            dBdTSpec[n] = invRu
                * ((c[8] - Ru) * invT + c[9] + T * (2 * c[10] + T * (3 * c[11] + T * 4 * c[12])) + c[13] * invT * invT);
          }
          else
          {
            specG[n] = c[6] + T * (c[1] - c[7] - c[1] * logT - T * (c[2] + T * (c[3] + T * (c[4] + T * c[5]))));
            dBdTSpec[n] = invRu
                * ((c[1] - Ru) * invT + c[2] + T * (2 * c[3] + T * (3 * c[4] + T * 4 * c[5])) + c[6] * invT * invT);
          }
          break;
        case CpType::CONST:
            ...
            if (sp_nonzero[j])
            {
              for (int l = 0; l < n_sp; ++l)
              {
                ...
                else
                {
                  if (sp_order[l] > 1)
                  {
                    mod_rate *= sp_order[l] * rho * sp_invmw[l]
                        * std::pow (std::max (C_S(l), 1.e-16), sp_order[l] - 1.);
                  }
                  else
                  {
                    mod_rate *= sp_order[l] * rho * sp_invmw[l]
                        / std::pow (std::max (C_S(l), 1.e-16), 1. - sp_order[l]);
                  }
                }
              }
              dRnetdY[s] = mod_rate;
        ...
        if (rxnData.reversible)
        {
          ...
          switch (n_net)
          {
            case 3:
              Kc =
                  std::exp (
                      -(sumStoich * std::log (standardStatePressure * invT * invRu)
                          - invT * invRu * (GRIFFON_SUM3(GIBBS))));
                ...
                default:
                  ...
                  switch (std::abs (pd_stoich[sridx]))
                  {
                    ...
                    case 3:
                      ...
                      break;
                    default:
                      nsTmp = kr * rho * pd_invmw[sridx] * std::abs (pd_stoich[sridx])
                          * std::pow (C_P(sridx), std::abs (pd_stoich[sridx]) - 1);
                      break;
                  }
...
#undef C_P
      ...
      switch (rxnData.type)
      {
        ...
        case RateType::TROE:
          ...
          dfTroedT = fTroe
              * (gTroe / fCent * dfCentdT
                  + logfcent
                      * (-2.0 * gTroe * gTroe / ln10 * aTroe / (bTroe * bTroe * bTroe)
                          * ((bTroe + 0.14 * aTroe) * (ARRHENIUS_SENS_OVER_K(
                              kPCoefs) - ARRHENIUS_SENS_OVER_K(kCoefs))
                              - (0.67 * bTroe - 1.1762 * aTroe) * dfCentdT / fCent)));
          dCtbafdT = 1. / (1. + 1. / pr) * dfTroedT
              + fTroe * pr / ((1. + pr) * (1. + pr)) * (ARRHENIUS_SENS_OVER_K(kPCoefs) - ARRHENIUS_SENS_OVER_K(kCoefs));

//          nsTmp = (-2.0 / (1. + 1. / pr) * fTroe * logfcent * gTroe * gTroe / ln10 * aTroe / (bTroe * bTroe * bTroe) * (bTroe + 0.14 * aTroe) + pr * fTroe / ((1. + pr) * (1 + pr)));
          nsTmp = kp_over_kf
              * (-2.0 / (1. + pr) * fTroe * logfcent * gTroe * gTroe / ln10 * aTroe / (bTroe * bTroe * bTroe)
                  * (bTroe + 0.14 * aTroe) + fTroe / ((1. + pr) * (1 + pr)));
          dCtbafdrho = nsTmp * baseEff * invM;
          for (int i = 0; i < n_tb; ++i)
            dCtbafdrho += nsTmp * THD_BDY(i);
