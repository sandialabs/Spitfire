...
from spitfire.chemistry.tabulation import build_unreacted_library, Library
from numpy.testing import assert_allclose
from os.path import join, abspath


class UnreactedFlameletLibrary(unittest.TestCase):
    def test(self):
        ...
        l1 = build_unreacted_library(flamelet_specs, quantities, verbose=False)

        file_name = abspath(join('spitfire_test',
                                 'unit',
                                 'tabulation',
                                 'gold_standards_test_' + 'unreacted_library',
                                 'library_gold.pkl'))
