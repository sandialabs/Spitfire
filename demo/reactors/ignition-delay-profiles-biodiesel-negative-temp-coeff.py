...
b = ChemicalMechanismSpec(cantera_xml='mechanisms/biodiesel-lu-lowT.xml', group_name='biodiesel-lu-lowT')

air = b.stream(stp_air=True)
h2 = b.stream('X', 'nc7h16:2, md:1, md9d:1')
phi = 1.0
...
for pressure, marker in zip(pressure_list, markers_list):
    ...
    for idx, temperature in enumerate(temperature_list):
        ...
        r = HomogeneousReactor(b, mix,
                               ...
                               'adiabatic',
                               'closed')
        tau_list[idx] = r.compute_ignition_delay_direct_griffon(first_time_step=1.e-9)
